...
CoopCutMultilabel::CoopCutMultilabel(string name_) :
		nodeNumber_(-1),
		edgeNumber_(-1),
		edgeClassNumber_(-1),
		labelNumber_(-1),
...
bool CoopCutMultilabel::readUnaryTermsFromFile(const char* fileName)
{
	...
	fread(&nx_, sizeof(int), 1, fp);
	fread(&ny_, sizeof(int), 1, fp);

	if (labelNumber_ <= 0 || nodeNumber_ <= 0 || nx_ <= 0 || ny_ <= 0) {
		fclose( fp );
		return false;
	}
...
bool CoopCutMultilabel::readPairwiseTermsFromFile(const char* fileName)
{
	...
	classThreshold_.resize(2*edgeClassNumber_ + 1, 0);
	...
	// we also need to establish maxDegree_;
	maxDegree_ = -1;
	std::vector<int> vecDegrees(nodeNumber_,0);
	...
	for(int iEdge = 0; iEdge < edgeNumber_; ++iEdge) {
		...
		for(int degreeIter=0; degreeIter<2; ++degreeIter) {
			vecDegrees[iNode[degreeIter]]++;
			if (vecDegrees[iNode[degreeIter]] > maxDegree_) {
				maxDegree_ = vecDegrees[iNode[degreeIter]];
			}
		}
	...
	cout << "maxDegree_=" << maxDegree_ << endl;
	for(int iClass = 0; iClass < edgeClassNumber_; ++iClass)
		classThreshold_[iClass] *= edgeClassTheta_;	
...
void CoopCutMultilabel::setUpNeighborhood(GCoptimizationGeneralGraph* gc) {
	int node1;
	for(int node0=0; node0<nodeNumber_; ++node0) {
		// iterate over node0's neighbors.
		for(int neighborIdx=0; neighborIdx<neighborNumber_[node0]; ++neighborIdx) {
			node1 = neighboringNode_[node0][neighborIdx];
			if (node0<node1) { // it should be called only once, so.
				gc -> setNeighbors(node0, node1);
			}
		}
	}
}
...
bool CoopCutMultilabel::mimimizeEnergy_greedy()
{
	...
	cout << "Initial energy is " << energy_ << endl;
	...
	for(int iIter = 0; iIter < greedyMaxIter_; ++iIter) {
		...
		for(int iLabel = 0; iLabel < labelNumber_; ++iLabel)
			for(int iEdgeClass = 0; iEdgeClass < edgeClassNumber_; ++ iEdgeClass)
			{
				...
				save_ground_labels(iIter, iLabel, iEdgeClass);
			}
...
double CoopCutMultilabel::computeCoopEnergy(const std::vector<int> labeling)
{
	...
	vector<vector<double>> classCost(labelNumber_, vector<double>(2*edgeClassNumber_ + 1, 0) );
	...
	double pairwise = 0;
	for(int iLabel = 0; iLabel < labelNumber_; ++iLabel)
		for(int iEdgeClass = 0; iEdgeClass < 2*edgeClassNumber_ + 1; ++ iEdgeClass)
		{
			double b = (1 - alpha_) * classThreshold_[iEdgeClass];

			double curW = classCost[iLabel][iEdgeClass];
			
			pairwise += min( curW, curW * alpha_ + b);
		}
...
void CoopCutMultilabel::save_ground_labels(int iterationNumber, int iLabel, int iEdgeClass) {
	return;
	...
	img.normalize(0,255);
	string filename = "logs/greedy_iteration/" + name + "/labels"
		+ to_string(iterationNumber) + "." + to_string(iLabel) + "." + to_string(iEdgeClass) + ".params." 
		+ "0.0.1.bmp";
	// create the directory!
	string directory_temp = "logs/greedy_iteration/" + name + "/";
